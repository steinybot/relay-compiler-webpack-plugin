...
class RelayCompiler {
  ...
  runOnce() {
    var _a, _b;
    this.error = void 0;
    const subprocess = spawn__default["default"].sync(RELAY, this.args);
  ...
  watch(callback) {
    var _a, _b, _c, _d;
    if (!this.subprocess) {
      this.subprocess = spawn__default["default"](RELAY, [...this.args, "--watch"]);
...
const checkWatchman = () => {
  const { error } = spawn__default["default"].sync(WATCHMAN, ["-v"]);
  if (error) {
...
const _RelayCompilerPlugin = class {
  constructor(options) {
    ...
    schemaUtils.validate(schema, merged, { name: PLUGIN_NAME });
    this.options = merged;
...
exports.OutputKind = OutputKind;
exports.RelayCompilerPlugin = RelayCompilerPlugin;
